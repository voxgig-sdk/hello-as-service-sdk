...
// HelloAsService SDK retry feature
...
class HelloAsServiceRetryFeature extends HelloAsServiceBaseFeature
{
    ...
    public function init(HelloAsServiceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HelloAsServiceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HelloAsServiceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HelloAsServiceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
