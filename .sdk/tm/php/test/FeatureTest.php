...
// HelloAsService SDK feature test
//
...
class FtCtrl extends HelloAsServiceControl
{
...
class FtClient
{
    ...
    private HelloAsServiceUtility $utility;

    public function __construct(HelloAsServiceUtility $utility, array $options)
    {
    ...
    public function get_utility(): HelloAsServiceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HelloAsServiceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HelloAsServiceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HelloAsServiceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HelloAsServiceConfig::make_feature($name);
    ...
    private static function buildUrl(HelloAsServiceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HelloAsServiceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HelloAsServiceResponse($fetched);
        }
    ...
    private function fail(HelloAsServiceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HelloAsServiceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
