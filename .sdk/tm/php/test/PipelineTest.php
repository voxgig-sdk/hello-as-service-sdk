...
// HelloAsService SDK pipeline test
//
...
{
    private static function utility(): HelloAsServiceUtility
    {
        return new HelloAsServiceUtility();
    }
    ...
    private static function ctx(array $over = []): HelloAsServiceContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new HelloAsServiceContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new HelloAsServiceOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof HelloAsServiceError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new HelloAsServiceOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = HelloAsServiceMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new HelloAsServiceOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = HelloAsServiceMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new HelloAsServiceOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = HelloAsServiceMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new HelloAsServiceSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = HelloAsServiceMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new HelloAsServiceResponse([]),
            'result' => new HelloAsServiceResult([]),
        ]);
        [, $err] = HelloAsServiceMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new HelloAsServiceSpec([]),
            'response' => null,
            'result' => new HelloAsServiceResult([]),
        ]);
        [, $err] = HelloAsServiceMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new HelloAsServiceSpec(['step' => 's']),
            'response' => new HelloAsServiceResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new HelloAsServiceResult([]),
        ]);
        [, $err] = HelloAsServiceMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new HelloAsServiceSpec(['step' => 's']),
            'response' => new HelloAsServiceResponse(self::resp(200, ['v' => 1])),
            'result' => new HelloAsServiceResult([]),
        ]);
        [, $err] = HelloAsServiceMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new HelloAsServiceSpec(['step' => 's']),
            'response' => new HelloAsServiceResponse(self::resp(200, ['v' => 2])),
            'result' => new HelloAsServiceResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        HelloAsServiceMakeResponse::call($ctx);
    ...
    {
        $preset = new HelloAsServiceResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new HelloAsServiceSpec([]),
            'response' => new HelloAsServiceResponse([]),
            'result' => new HelloAsServiceResult([]),
        ...
        [$got, $err] = HelloAsServiceMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new HelloAsServiceResult([])]);
        [, $err] = HelloAsServiceMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new HelloAsServiceSpec([]), 'result' => null]);
        [, $err] = HelloAsServiceMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new HelloAsServiceSpec(['step' => 's']),
            'result' => new HelloAsServiceResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new HelloAsServiceOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = HelloAsServiceMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new HelloAsServiceSpec(['step' => 's']),
            'result' => new HelloAsServiceResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new HelloAsServiceOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = HelloAsServiceMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new HelloAsServiceResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new HelloAsServiceSpec([]),
            'result' => new HelloAsServiceResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = HelloAsServiceMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = HelloAsServiceMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new HelloAsServiceError('boom', 'boom');
        $utility->fetcher = function (HelloAsServiceContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new HelloAsServiceSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = HelloAsServiceMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (HelloAsServiceContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = HelloAsServiceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(HelloAsServiceResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        HelloAsServiceMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (HelloAsServiceContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new HelloAsServiceSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = HelloAsServiceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new HelloAsServiceResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new HelloAsServiceSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = HelloAsServiceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = HelloAsServiceMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = HelloAsServiceMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new HelloAsServiceResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, HelloAsServiceDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new HelloAsServiceResult(['ok' => false])]);
        $this->expectException(HelloAsServiceError::class);
        HelloAsServiceDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new HelloAsServiceResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', HelloAsServiceMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        HelloAsServiceMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new HelloAsServiceBaseFeature();
        $b = new HelloAsServiceBaseFeature();
        HelloAsServiceFeatureAdd::call($ctx, $a);
        HelloAsServiceFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): HelloAsServiceBaseFeature
    {
        $f = new HelloAsServiceBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        HelloAsServiceFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): HelloAsServiceContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new HelloAsServiceSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = HelloAsServicePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        HelloAsServicePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new HelloAsServiceResponse(['headers' => null]),
            'result' => new HelloAsServiceResult([]),
        ]);
        HelloAsServiceResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        HelloAsServiceResultBody::call($ctx);
