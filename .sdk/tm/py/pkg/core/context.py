...
from projectname_sdk.core.control import HelloAsServiceControl
...
class HelloAsServiceContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = HelloAsServiceControl()
        ...
        if isinstance(sp, HelloAsServiceSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, HelloAsServiceResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, HelloAsServiceResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return HelloAsServiceOperation({})
    ...
    def make_error(self, code, msg):
        return HelloAsServiceError(code, msg, self)
