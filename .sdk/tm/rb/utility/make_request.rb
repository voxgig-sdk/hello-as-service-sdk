...
module HelloAsServiceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HelloAsServiceResponse.new({})
    ...
    if fetch_err
      ...
      response = HelloAsServiceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HelloAsServiceResponse.new(fetched)
    else
