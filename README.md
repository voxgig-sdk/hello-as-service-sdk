...
This API allows developers to greet visitors in their native language by utilizing their IP address or browser language settings.

Learn more about Hello as Service at [hellosalut.stefanbohacek.dev](https://hellosalut.stefanbohacek.dev).
...
const getgreeting = await client.GetGreeting().load()
// getgreeting is the GetGreeting entity, populated with mock data
...
// Load a specific getgreeting (returns the ENTITY; call data_get() for the record; throws on error)
$getgreeting = $client->GetGreeting()->load();
...
# Load a specific getgreeting (returns the ENTITY; call data_get for the record)
getgreeting = client.GetGreeting.load()
puts getgreeting
```
