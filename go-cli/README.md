...
boru-driven command-line client **and** interactive REPL for the HelloAsService
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hello-as-service-cli)
make build

# 2. See usage (words, entities, env vars)
./hello-as-service-cli --help

# 3. Provide credentials once, via the environment
export HELLO_AS_SERVICE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./hello-as-service-cli load 1 get_greeting            # {id:1} shorthand
./hello-as-service-cli load '{id:1}' get_greeting       # explicit match map

# 5. Override the API base URL for a single call
HELLO_AS_SERVICE_BASE=https://api.example.com ./hello-as-service-cli load 1 get_greeting

# 6. No arguments -> interactive REPL
./hello-as-service-cli
hello-as-service> load 1 get_greeting
hello-as-service> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hello-as-service-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/hello-as-service-cli load 1 get_greeting
   ```
...
```sh
./hello-as-service-cli load 1 get_greeting          # scalar shorthand for {id:1}
./hello-as-service-cli load '{id:1}' get_greeting     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `hello-as-service>`). Each line is
evaluated as its own boru expression:

```text
$ ./hello-as-service-cli
hello-as-service> load 1 get_greeting
hello-as-service> /help
hello-as-service> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_greeting`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/hello-as-service-cli`. |
...
(`./hello-as-service-cli <expr>`) and interactively (the REPL), and expressions compose the
