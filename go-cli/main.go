// Command HelloAsService-cli is a CLI + REPL for the HelloAsService SDK,
...
const prompt = "hello-as-service"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_greeting"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HELLO_AS_SERVICE_APIKEY (api key), HELLO_AS_SERVICE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HELLO_AS_SERVICE_APIKEY carries the API key and
	// HELLO_AS_SERVICE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HELLO_AS_SERVICE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HELLO_AS_SERVICE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHelloAsServiceSDK(opts)
