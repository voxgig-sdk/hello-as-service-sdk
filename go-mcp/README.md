# hello-as-service-mcp

[MCP](https://modelcontextprotocol.io) server exposing the HelloAsService SDK as
two agent tools — `hello-as-service_list` and `hello-as-service_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hello-as-service-mcp)
make build

# 2. Provide credentials via the environment
export HELLO_AS_SERVICE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user hello-as-service \
  -- /absolute/path/to/hello-as-service-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./hello-as-service-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// hello-as-service_list: first page of records
{ "entity": "get_greeting" }
{ "entity": "get_greeting", "query": { } }

// hello-as-service_load: one record by id
{ "entity": "get_greeting", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hello-as-service-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hello-as-service \
     -- "$PWD"/dist/*/hello-as-service-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `hello-as-service_list` and `hello-as-service_load` tools now appear
   in new sessions. Ask the agent to *"list get_greeting using hello-as-service"*
   and it calls `hello-as-service_list` with `{"entity":"get_greeting"}`.
...
```sh
export HELLO_AS_SERVICE_APIKEY=sk_live_xxx            # API key
export HELLO_AS_SERVICE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `hello-as-service_list` tool
...
### Call the `hello-as-service_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `hello-as-service_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hello-as-service_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HELLO_AS_SERVICE_APIKEY` | API key sent with every request. |
| `HELLO_AS_SERVICE_BASE` | Optional override of the API base URL. |
...
```sh
./hello-as-service-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"hello-as-service_load","arguments":{"entity":"get_greeting","query":{"id":1}}}}'
```
