...
import (
	...
	omni "github.com/voxgig-sdk/hello-as-service-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HelloAsServiceError); is {
				out := map[string]any{
					"name":    "HelloAsServiceError",
...
func omniCtx(arg any, client *sdk.HelloAsServiceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HelloAsServiceSDK); is {
			client = live
