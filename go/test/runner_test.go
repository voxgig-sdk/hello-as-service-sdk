...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("HELLO_AS_SERVICE_TEST_LIVE") == "TRUE" ||
		os.Getenv("HELLO_AS_SERVICE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("HELLO_AS_SERVICE_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.HelloAsServiceSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.HelloAsServiceError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.HelloAsServiceSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.HelloAsServiceError{Msg: msg, Code: code}
}
