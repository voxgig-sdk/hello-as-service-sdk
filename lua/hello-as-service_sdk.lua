...
require("hello-as-service_types")
...
local HelloAsServiceSDK = {}
HelloAsServiceSDK.__index = HelloAsServiceSDK
...
HelloAsServiceSDK._make_feature = _make_feature


function HelloAsServiceSDK.new(options)
  local self = setmetatable({}, HelloAsServiceSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function HelloAsServiceSDK:options_map()
...
function HelloAsServiceSDK:get_utility()
  return Utility.copy(self._utility)
end


function HelloAsServiceSDK:get_root_ctx()
  return self._rootctx
end


function HelloAsServiceSDK:prepare(fetchargs)
...
function HelloAsServiceSDK:direct(fetchargs)
...
-- by passing it.
function HelloAsServiceSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function HelloAsServiceSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "HelloAsServiceSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function HelloAsServiceSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = HelloAsServiceSDK.new(sdkopts)
  sdk.mode = "test"
