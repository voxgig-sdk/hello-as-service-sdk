...
// HelloAsService SDK GetGreeting entity
...
class GetGreetingEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetGreetingEntity($this->_client, $opts);
    }

    /**
     * @param GetGreeting|array $args GetGreeting data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = HelloAsServiceHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetGreeting|array The current GetGreeting data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetGreeting fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = HelloAsServiceHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetGreeting fields).
     */
    ...
    /**
     * Load a single GetGreeting.
     *
     * @param GetGreetingLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetGreetingLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetGreeting|array The loaded GetGreeting as an assoc-array at the
     *   SDK boundary; throws HelloAsServiceError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = HelloAsServiceHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
