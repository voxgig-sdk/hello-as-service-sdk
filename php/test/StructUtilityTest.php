...
// HelloAsService SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = HelloAsServiceOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(HelloAsServiceOmni::structfix($now, Struct::undef()))
                        !== json_encode(HelloAsServiceOmni::structfix($val, Struct::undef()))) {
