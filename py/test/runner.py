# HelloAsService SDK test runner
...
class HelloAsServiceTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HelloAsServiceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HelloAsServiceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HelloAsServiceTestRunner.getenv("HELLO_AS_SERVICE_TEST_LIVE")
        override = HelloAsServiceTestRunner.getenv("HELLO_AS_SERVICE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HelloAsServiceTestRunner.getenv(key)
        ...
        explain = HelloAsServiceTestRunner.getenv("HELLO_AS_SERVICE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if HelloAsServiceTestRunner._test_control is not None:
            return HelloAsServiceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HelloAsServiceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HelloAsServiceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HelloAsServiceTestRunner.load_test_control()
...
def load_env_local():
    HelloAsServiceTestRunner.load_env_local()


def env_override(m):
    return HelloAsServiceTestRunner.env_override(m)


def entity_data(v):
    return HelloAsServiceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HelloAsServiceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HelloAsServiceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HelloAsServiceTestRunner.load_test_control()


def live_client_options():
    return HelloAsServiceTestRunner.live_client_options()


def live_delay_ms():
    return HelloAsServiceTestRunner.live_delay_ms()
