...
require_relative 'HelloAsService_types'
...
class HelloAsServiceSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = HelloAsServiceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = HelloAsServiceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, HelloAsServiceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    HelloAsServiceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = HelloAsServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = HelloAsServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = HelloAsServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue HelloAsServiceError => err
    ...
    if fetched.is_a?(Hash)
      status = HelloAsServiceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = HelloAsServiceError.new(
        "graphql_error", "HelloAsServiceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = HelloAsServiceSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
