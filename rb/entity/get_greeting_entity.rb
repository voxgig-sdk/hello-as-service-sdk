...
class GetGreetingEntity
  ...
  def make
    opts = @_entopts.dup
    GetGreetingEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HelloAsServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetGreeting, Hash] the current GetGreeting data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = HelloAsServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetGreeting fields)
  def match_get
  ...
  #
  # @param reqmatch [GetGreetingLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetGreeting.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetGreeting, Hash] the loaded GetGreeting; raises HelloAsServiceError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = HelloAsServiceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
