...
class GetGreetingEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = get_greeting_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "get_greeting." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set HELLO_AS_SERVICE_TEST_GET_GREETING_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    get_greeting_ref01_data = nil
    if get_greeting_ref01_data_raw.length > 0
...
def get_greeting_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "get_greeting", "GetGreetingTestData.json")
  ...
  entid_env_raw = ENV["HELLO_AS_SERVICE_TEST_GET_GREETING_ENTID"]
  ...
  if env["HELLO_AS_SERVICE_TEST_LIVE"] == "TRUE"
    ...
    client = HelloAsServiceSDK.new(Helpers.to_map(merged_opts))
  end
