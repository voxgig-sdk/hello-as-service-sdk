...
//     (\`new HelloAsServiceSDK(...)\` / \`HelloAsServiceSDK.test(...)\` become
//     \`HelloAsServiceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'HelloAsServiceSDK'
...
const TEST_SEED = {"entity":{"get_greeting":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HelloAsServiceSDK(...)\` and any
// \`HelloAsServiceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HelloAsServiceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HelloAsServiceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
