...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.HELLOASSERVICE_TEST_LIVE ||
    'TRUE' === process.env.HELLOASSERVICE_TEST_OVERRIDE
  ) {
  ...
  m.HELLOASSERVICE_TEST_EXPLAIN = process.env.HELLOASSERVICE_TEST_EXPLAIN || m.HELLOASSERVICE_TEST_EXPLAIN
